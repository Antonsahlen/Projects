...
public class HR3_2_OwnerCollectionTest {
	 ...
	 * för getOwner.
	 */
	private Owner getOwnerFromCollection(String name) {
		Object possibleOwner = collection.getOwner(name);

		if (possibleOwner instanceof Optional<?> optional) {
			possibleOwner = optional.orElse(null);
		}

		if (possibleOwner instanceof Owner owner)
			return owner;

		if (possibleOwner == null)
			return null;

		throw new ClassCastException("Fel returtyp för getOwner: " + possibleOwner.getClass());
	}
	@Test
	@Order(220)
	...
	public void attemptingToGetOwnerFromEmptyCollection() {
		...
		var dog = getOwnerFromCollection(OTHER_OWNER.getName());
	...
	public void getOnlyOwner() {
		...
		var dog = getOwnerFromCollection(A_OWNER.getName());
	...
	public void getFirstOwner() {
		addAllThreeTestOwnersInOrder();
		var dog = getOwnerFromCollection(FIRST_OWNER.getName());
	...
	public void getSecondOwner() {
		addAllThreeTestOwnersInOrder();
		var dog = getOwnerFromCollection(SECOND_OWNER.getName());
	...
	public void getLastOwner() {
		addAllThreeTestOwnersInOrder();
		var dog = getOwnerFromCollection(THIRD_OWNER.getName());
	...
	public void getOwnerNewStringObject() {
		addAllThreeTestOwnersInOrder();
		var dog = getOwnerFromCollection(new String(A_OWNER.getName()));
		assertEquals(A_OWNER, dog);
	}
	...
	public class OwnerCollectionImplementationValidator {
		...
		static {
			...
			EXPECTED_PUBLIC_METHODS.add(new MethodHeader(false, "Iterator", "iterator"));
			EXPECTED_PUBLIC_METHODS.add(new MethodHeader(false, "Optional", "getOwner", "String"));
