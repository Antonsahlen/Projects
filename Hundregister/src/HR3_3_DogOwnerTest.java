...
public class HR3_3_DogOwnerTest {
	...
	/**
	 * Nödvändig eftersom uppgiften tillåter olika returtyper för Dog.getOwner.
	 */
	private void assertOwnerIs(Dog dog, Owner expectedOwner) {
		Object possibleOwner = dog.getOwner();
		if (possibleOwner instanceof Optional<?> optional) {
			possibleOwner = optional.orElse(null);
		}
		Owner actualOwner = (Owner) possibleOwner;

		assertEquals(expectedOwner, actualOwner, "Fel ägare för " + dog.getName());
	}
	...
	public void settingAndRemovingOwnerMultipleTimes() {
		...
		assertOwnerIs(dog, firstOwner);

		dog.setOwner(null);
		assertOwnerIs(dog, null);

		dog.setOwner(secondOwner);
		assertOwnerIs(dog, secondOwner);
