...
public class HR1_2_DogTest {
	...
	public static final String VERSION = "2024-11-15 16:36";
	...
	private static final Method ASSUMED_METHOD_FOR_INCREASING_AGE_OF_DOG = guessUnnamedMethod(
			asArray(void.class, int.class), asArray(Void.class, int.class));
	...
	private static void callAssumedMethodForIncreasingAgeOfDog(Dog dog, int years) {
		if (ASSUMED_METHOD_FOR_INCREASING_AGE_OF_DOG == null)
			fail("""
	...
	public void fullNames() {
		...
		assertNameIsNormalized(dog, "Fluffy destroyer of worlds", "Fluffy Destroyer Of Worlds",
	...
	public void doubleNameBreed() {
		...
		assertBreedIsNormalized(dog, "Anglo-russkaja gontjaja", "Anglo-russkaja Gontjaja", "Anglo-Russkaja Gontjaja",
	...
	public void unicodeCharsInBreed() {
		...
		assertBreedIsNormalized(dog, "Dvärgschnauzer, peppar & salt", "Dvärgschnauzer, Peppar & Salt",
		...
		assertBreedIsNormalized(dog, "Slovenský hrubosrsty stavac (ohar)", "Slovenský Hrubosrsty Stavac (ohar)",
				"Slovenský Hrubosrsty Stavac (Ohar)", "SLOVENSKÝ HRUBOSRSTY STAVAC (OHAR)");
	}
	...
	public void randomDogHasCorrectAttribute(Dog dog, String expectedName, String expectedBreed, int expectedAge,
			int expectedWeight, double expectedTailLength) {
		...
		assertAll(() -> assertEquals(expectedName.toLowerCase(), dog.getName().toLowerCase(), "Namnet är fel"),
				() -> assertEquals(expectedBreed.toLowerCase(), dog.getBreed().toLowerCase(), "Rasen är fel"),
				() -> assertEquals(expectedAge, dog.getAge(), "Åldern är fel"),
				() -> assertEquals(expectedWeight, dog.getWeight(), "Vikten är fel"),
				() -> assertEquals(expectedTailLength, dog.getTailLength(), 0.01, "Svanslängden är fel (±0.01)"));
	}

	private static Stream<Arguments> randomDogs() {
		return Stream.of(Arguments.of(new Dog("Snobben", "Pudel", 14, 18), "Snobben", "Pudel", 14, 18, 25.2),
				Arguments.of(new Dog("Devil", "Dachshund", 15, 11), "Devil", "Dachshund", 15, 11, 3.7),
				Arguments.of(new Dog("Sigge", "Golden retriever", 1, 12), "Sigge", "Golden retriever", 1, 12, 1.2),
				Arguments.of(new Dog("Wilma", "Beagle", 19, 13), "Wilma", "Beagle", 19, 13, 24.7),
				Arguments.of(new Dog("Doris", "Dachshund", 14, 5), "Doris", "Dachshund", 14, 5, 3.7),
				Arguments.of(new Dog("Ronja", "Shih tzu", 10, 6), "Ronja", "Shih tzu", 10, 6, 6.0),
				Arguments.of(new Dog("Rex", "Chihuahua", 11, 14), "Rex", "Chihuahua", 11, 14, 15.4),
				Arguments.of(new Dog("Bamse", "Tax", 10, 8), "Bamse", "Tax", 10, 8, 3.7),
				Arguments.of(new Dog("Ludde", "Border collie", 13, 12), "Ludde", "Border collie", 13, 12, 15.6),
				Arguments.of(new Dog("Lassie", "Yorkshireterrier", 1, 6), "Lassie", "Yorkshireterrier", 1, 6, 0.6));
	}
	...
	public void onlyPublicOrPrivateMethodsInDog() {
		...
		for (var m : c.getDeclaredMethods()) {
			assertTrue(isPublic(m) || isPrivate(m),
					"Metoden %s i %s är inte publik eller privat".formatted(m.getName(), c.getSimpleName()));
		}
	}
	...
	public void onlyAllowedPublicMethodsInDog() {
		...
		assertTrue(matches.isEmpty(),
				"Det finns publika metoder i Dog som testen inte förväntar sig:\n%s".formatted(matches));
	}
	...
	private static Collection<Method> completelySpecifiedMethodsInDog() {
		...
		matches.addAll(getMatchingPublicMethodsInDog(true, asArray(Owner.class, Optional.class), "getOwner",
				asArray(Void.class)));
		matches.addAll(getMatchingPublicMethodsInDog(true, asArray(boolean.class, void.class), "setOwner",
				asArray(Owner.class)));
	...
	private static Collection<Method> getMatchingPublicMethodsInDog(boolean instanceMethod, Class<?> allowedReturnType,
			String name, Class<?> allowedParamType) {
		return getMatchingPublicMethodsInDog(instanceMethod, asArray(allowedReturnType), name,
				asArray(allowedParamType));
	}
	 ...
	 */
	private static Collection<Method> getMatchingPublicMethodsInDog(boolean instanceMethod,
			Class<?>[] allowedReturnTypes, String name, Class<?>[] allowedParamTypes) {
		var matches = new HashSet<Method>();
		for (var method : allPublicMethodsInDog()) {
			...
			var paramTypeMatch = (method.getParameterCount() == 0 && matchingType(Void.class, allowedParamTypes))
					|| (method.getParameterCount() == 1
							&& matchingType(method.getParameters()[0].getType(), allowedParamTypes));
	...
	public class DogImplementationValidator {
		 ...
		 * Ordningen på metoderna har inget med den förväntade ordningen i klassen
		 * att göra.
		 */
		static {
